...
class EutesterTestCase(unittest.TestCase):
    ...
    def setuptestcase(self,
        ...
        if use_default_file:
            # first add $USERHOME/.nephoria/nephoria.conf if it exists
    ...
    def setup_parser(self,
                     ...
                     region=True):
        '''
        Description: Convenience method to setup argparse parser and some canned default arguments,
        based upon the boolean values provided. For each item marked as 'True' this method will
        ...
        :type emi: boolean
        :param emi: Flag to present the emi command line argument/option for providing an image
                    emi id via the cli

        :type zone: boolean
        :param zone: Flag to present the zone command line argument/option for providing a zone
                    via the cli

        :type vmtype: boolean
        :param vmtype: Flag to present the vmtype command line argument/option for providing a
                       vmtype via the cli

        :type keypair: boolean
        :param kepair: Flag to present the keypair command line argument/option for providing a
                      keypair via the cli

        :type credpath: boolean
        :param credpath: Flag to present the credpath command line argument/option for providing
                         a local path to creds via the cli
        ...
        :param userdata: Flag to present the userdata command line argument/option for providing
                         userdata to instance(s) within test

        :type instance_user: boolean
        :param instance_user: Flag to present the instance_user command line argument/option
                              for providing an ssh username for instance login via the cli

        :type instance_password: boolean
        :param instance_password: Flag to present the instance_password command line
        ...
        :param logfile_level: boolean flag to present the --logfile_level command line option to
                              set the log file log level

        :param logfile: boolean flag to present the --logfile command line option to set the log
                        file path to log to

        :param region: boolean flag to present the --region command line option to set the region
                       for the test to use
        '''
    ...
    def setup_debugmethod(self, testcasename=None, log_level=None, logfile=None,
                          logfile_level=None):
    ...
    def debug(self, msg, traceback=1, color=None, linebyline=True):
        ...
        param traceback: integer value for what frame to inspect to derive the originating method
                         and method line number
    ...
    def create_testunit_from_method(self, method, *args, **kwargs):
        ...
        if self.args.html_anchors:
            testunit = EutesterTestUnit(method, *args, html_anchors=self.args.html_anchors,
                                        **kwargs)
        else:
            testunit = EutesterTestUnit(method, *args, **kwargs)
        testunit.eof = eof
    ...
    def status(self, msg, traceback=2, b=1, a=0, testcolor=None):
        ...
        :type traceback: integer
        :param traceback: integer value for what frame to inspect to derive the originating
                          method and method line number
    ...
    def print_test_unit_startmsg(self, test):
        ...
        startbuf += 'Running list method: "' + str(
            self.print_testunit_method_arg_values(test)) + '"'
        if self.args.html_anchors:
            startbuf += '\n </div>'
    ...
    def print_test_list_results(self, list=None, printout=True, printmethod=None):
        ...
        if printmethod is None:
            def printmethod(msg):
                self.debug(msg, linebyline=False)
        ...
        for testunit in list:
            ...
            if testunit.result != EutesterTestResult.passed:
                pmethod = self.resultfail
            else:
                pmethod = self.resultdefault
    ...
    def get_args(self, use_cli=True, file_sections=[]):
        ...
        for conf in euconfigs:
            ...
            # If globals are still in our confblocks, add globals first if the section is
            # present in config
        ...
        if cliargs:
            ...
            args = cf
        # Legacy script support: level set var names for config_file vs configfile vs config
        # and credpath vs cred_path
        try:
            if 'config' in args:
        ...
        if not self.has_arg('args'):
            args.__setattr__('args', copy.copy(args))
            ...
            for methvar in method_args:
                if methvar == apply_val[0]:
                    ...
                    if apply_val[0] in testunit_obj_args:
                        self.debug("Skipping populate because testunit already has this arg:" +
                                   str(methvar))
                        continue
